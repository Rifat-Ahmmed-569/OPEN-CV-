"""Live stereo stream edge detection with OpenCV.
...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description="Display live Canny edge detection for a stereo camera pair."
	)
	parser.add_argument(
		"--left",
		type=int,
...
def main() -> int:
	...
	try:
		...
		cv2.namedWindow("Stereo edge detection", cv2.WINDOW_NORMAL)
		...
		while True:
			...
			if single_mode:
				left_view = add_label(first_frame, "CAMERA")
				edges = detect_edges(
			...
			if not single_mode:
				...
				left_view = add_label(left_edges, "LEFT edges")
				right_view = add_label(right_edges, "RIGHT edges")

			stereo_view = cv2.hconcat([left_view, right_view])
			cv2.imshow("Stereo edge detection", stereo_view)
